...
import type { NounDeclension, Phrase } from '../types.ts';
...
const NounDeclensionModal: React.FC<NounDeclensionModalProps> = ({
  isOpen,
  onClose,
  noun,
  data,
  ...
  const renderContent = () => {
      ...
      return (
        <div className="flex justify-center items-center h-full">
          <div className="text-center bg-red-900/50 border border-red-700 text-red-300 p-4 rounded-lg">
            <p className="font-semibold">{t('modals.nounDeclension.errors.generic')}</p>
    ...
    if (!data) {
      return (
        <div className="flex justify-center items-center h-full">
          <p className="text-slate-400">{t('modals.nounDeclension.errors.noData')}</p>
        </div>
      );
    }
    ...
    const caseInfo: { [key in CaseKey]: { name: string; question: string } } = {
      nominativ: {
        name: t('modals.nounDeclension.cases.nominativ.name'),
        question: t('modals.nounDeclension.cases.nominativ.question'),
      },
      akkusativ: {
        name: t('modals.nounDeclension.cases.akkusativ.name'),
        question: t('modals.nounDeclension.cases.akkusativ.question'),
      },
      dativ: {
        name: t('modals.nounDeclension.cases.dativ.name'),
        question: t('modals.nounDeclension.cases.dativ.question'),
      },
      genitiv: {
        name: t('modals.nounDeclension.cases.genitiv.name'),
        question: t('modals.nounDeclension.cases.genitiv.question'),
      },
    };
          ...
          <thead>
            <tr className="border-b border-slate-600">
              ...
              <th className="p-3 text-sm font-semibold text-slate-400">
                {t('modals.nounDeclension.headers.singular')}
              </th>
          ...
          <tbody>
            {cases.map((caseKey) => (
              <tr key={caseKey} className="border-b border-slate-700 last:border-b-0">
                ...
                <td className="p-3 text-slate-100 font-medium">{renderCellContent(data.singular[caseKey])}</td>
                <td className="p-3 text-slate-100 font-medium">{renderCellContent(data.plural[caseKey])}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-[70] flex justify-center items-center" onClick={onClose}>
      ...
      >
        ...
        <div className="p-2 overflow-y-auto">{renderContent()}</div>
      </div>
    </div>
  );
};
