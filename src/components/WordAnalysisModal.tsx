...
const WordAnalysisModal: React.FC<WordAnalysisModalProps> = ({
  ...
  const renderContent = () => {
      ...
      return (
        <div className="flex justify-center items-center h-full">
          <div className="text-center bg-red-900/50 border border-red-700 text-red-300 p-4 rounded-lg">
            <p className="font-semibold">{t('modals.wordAnalysis.errors.generic')}</p>
    ...
    if (!analysis) {
      return (
        <div className="flex justify-center items-center h-full">
          <p className="text-slate-400">{t('modals.wordAnalysis.errors.unknown')}</p>
        </div>
      );
    }
      ...
      <>
        ...
        <div className="text-center sticky top-0 bg-slate-800 pt-2 pb-2 z-10">
          <div className="flex justify-center items-center gap-x-3">
            <h4 className="text-xl font-bold text-slate-100">{analysis.word}</h4>
            <AudioPlayer textToSpeak={analysis.word} />
          </div>
          <p className="text-m text-purple-300 mt-1">{analysis.partOfSpeech}</p>
        </div>
            ...
            <div className="bg-slate-700/50 px-2 py-1 rounded-lg space-y-3">
              ...
              {analysis.baseForm && (
                <div className="flex justify-between items-center">
                  <span className="text-slate-400">{t('modals.wordAnalysis.labels.baseForm')}:</span>{' '}
                  <strong className="text-slate-100">{analysis.baseForm}</strong>
                </div>
              )}
              {analysis.nounDetails && (
                <>
                  <div className="flex justify-between items-center">
                    <span className="text-slate-400">{t('modals.wordAnalysis.labels.article')}:</span>{' '}
                    <strong className="text-slate-100 font-mono bg-slate-600 px-2 py-0.5 rounded">
                      {analysis.nounDetails.article}
                    </strong>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-slate-400">{t('modals.wordAnalysis.labels.plural')}:</span>{' '}
                    <strong className="text-slate-100">{analysis.nounDetails.plural}</strong>
                  </div>
                </>
              )}
              {analysis.verbDetails && (
                <>
                  <div className="flex justify-between items-center">
                    <span className="text-slate-400">{t('modals.wordAnalysis.labels.infinitive')}:</span>{' '}
                    <strong className="text-slate-100">{analysis.verbDetails.infinitive}</strong>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-slate-400">{t('modals.wordAnalysis.labels.tense')}:</span>{' '}
                    <strong className="text-slate-100">{analysis.verbDetails.tense}</strong>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-slate-400">{t('modals.wordAnalysis.labels.person')}:</span>{' '}
                    <strong className="text-slate-100">{analysis.verbDetails.person}</strong>
                  </div>
                </>
              )}
            </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                <p className="text-slate-200 text-m leading-relaxed">
                  "{renderClickableLearning(analysis.exampleSentence, analysis.exampleSentenceNative)}"
                </p>
                <p className="text-slate-400 italic mt-1">
                  {t('modals.wordAnalysis.labels.exampleTranslation')} {analysis.exampleSentenceNative}
                </p>
              </div>
            </div>
          </div>
      ...
      >
        <header className="flex items-center justify-between px-3 py-1 border-b border-slate-700 flex-shrink-0">
          <div className="flex items-center space-x-3">
            ...
            <h3 className="text-sm font-bold text-slate-100">{t('modals.wordAnalysis.title', { word })}</h3>
          </div>
          <button onClick={onClose} className="p-2 rounded-full hover:bg-slate-700">
            <CloseIcon className="w-6 h-6 text-slate-400" />
          </button>
        </header>
        <div className="flex-grow p-0 overflow-y-auto hide-scrollbar">{renderContent()}</div>
      </div>
    </div>
  );
};
