...
interface CategoryAssistantModalProps {
  ...
  category: Category;
  phrases: Phrase[];
  onGetAssistantResponse: (
    categoryName: string,
    existingPhrases: Phrase[],
    request: CategoryAssistantRequest,
    history?: ChatMessage[]
  ) => Promise<ChatMessage['assistantResponse']>;
...
const AssistantChatMessageContent: React.FC<{
  ...
  const handleAddSelected = async () => {
    ...
    const existingLearningTexts = new Set(categoryPhrases.map((p) => p.text.learning.trim().toLowerCase()));

    const uniqueCards = selected.filter((card) => !existingLearningTexts.has(card.learning.trim().toLowerCase()));
    ...
    return part.text.split(' ').map((word, i, arr) => (
      <span
        ...
        onClick={(e) => {
          ...
          if (cleaned) handleWordClick(part.text, cleaned, part.translation || '');
        }}
        ...
        onContextMenu={(e) => {
          ...
          const cleaned = word.replace(/[.,!?()"“”:;]/g, '');
          if (cleaned)
            onOpenContextMenu({ sentence: { learning: part.text, native: part.translation || '' }, word: cleaned });
        }}
          ...
          <div className="space-y-2 pt-2 border-t border-slate-600">
            {proposedCards.map((card, index) => (
              <div
                ...
                className={`p-2 rounded-md flex items-start space-x-3 transition-colors ${!addedInfo && 'cursor-pointer bg-slate-800/50 hover:bg-slate-800'}`}
              >
                <div
                  className={`mt-1 w-5 h-5 rounded-md flex-shrink-0 flex items-center justify-center border-2 ${selectedIndices.has(index) ? 'bg-purple-600 border-purple-500' : 'bg-slate-900 border-slate-600'}`}
                >
            ...
            {addedInfo ? (
              <div className="flex items-center justify-between gap-2 pt-2">
                <p className="text-sm text-green-400">
                  {t('assistant.modal.success.added', { count: addedInfo.count })}
                </p>
                <button
                  onClick={() => {
                    onGoToList();
                  ...
                  className="px-3 py-1 text-sm bg-slate-600 hover:bg-slate-500 text-white font-semibold rounded-md flex items-center gap-2"
                >
              ...
              <button
                ...
                className="w-full mt-2 py-2 text-sm bg-purple-600 hover:bg-purple-700 text-white font-semibold rounded-md disabled:opacity-50 flex items-center justify-center min-h-[36px]"
              >
                {isAdding ? (
                  <Spinner className="w-5 h-5" />
                ) : (
                  t('assistant.modal.actions.addSelected', { count: selectedIndices.size })
                )}
              </button>
            )}
          </div>
        )}
        {(responseType === 'phrases_to_review' || responseType === 'phrases_to_delete') &&
          (phrasesToReview || phrasesForDeletion) && (
...
const CategoryAssistantModal: React.FC<CategoryAssistantModalProps> = (props) => {
  ...
  const initialActions = [
    {
      label: t('assistant.modal.quickActions.addSimilar'),
      icon: <WandIcon className="w-4 h-4" />,
      action: () => handleRequest({ type: 'add_similar' }),
    },
    {
      label: t('assistant.modal.quickActions.checkConsistency'),
      icon: <SearchIcon className="w-4 h-4" />,
      action: () => handleRequest({ type: 'check_homogeneity' }),
    },
    {
      label: t('assistant.modal.quickActions.createDialogue'),
      icon: <ChatIcon className="w-4 h-4" />,
      action: () => handleRequest({ type: 'create_dialogue' }),
    },
  ];

  const currentSuggestions =
    promptSuggestions.length > 0 ? promptSuggestions : messages.length <= 1 ? initialActions.map((a) => a.label) : [];
        ...
        >
          <header className="flex items-center justify-between p-4 border-b border-slate-700 flex-shrink-0">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="text-lg font-bold text-slate-100">
                {t('assistant.modal.title', { name: category.name })}
              </h2>
            </div>
            ...
            <div className="space-y-6">
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div
                    className={`max-w-[90%] px-4 py-3 rounded-2xl break-words ${msg.role === 'user' ? 'bg-purple-600 text-white rounded-br-lg' : 'bg-slate-700 text-slate-200 rounded-bl-lg'}`}
                  >
          ...
          <div className="p-4 border-t border-slate-700 flex-shrink-0 bg-slate-800/80 backdrop-blur-sm">
              ...
              <div className="flex flex-wrap justify-center gap-2 mb-3">
                {currentSuggestions.map((suggestion) => (
                  <button
                    ...
                    className="px-3 py-1.5 bg-slate-700/80 hover:bg-slate-700 text-slate-300 text-xs font-medium rounded-full transition-colors flex items-center justify-center gap-1.5"
                  >
            ...
            <form
              onSubmit={(e) => {
                e.preventDefault();
                handleRequest({ type: 'user_text', text: input });
              }}
            ...
            >
              <textarea
                ...
                placeholder={
                  isListening ? t('assistant.modal.input.listening') : t('assistant.modal.input.placeholder')
                }
              ...
              <button
                ...
                className={`p-3 rounded-lg transition-colors flex-shrink-0 ${isListening ? 'bg-red-600 animate-pulse' : 'bg-slate-600 hover:bg-slate-500'} disabled:bg-slate-600`}
              >
              ...
              <button
                ...
                className="p-3 bg-purple-600 rounded-lg hover:bg-purple-700 disabled:bg-slate-600"
              >
