...
const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const handleSettingChange = (
    setting: keyof Omit<Settings, "automation" | "enabledCategories">,
    value: boolean
  ) => {
  ...
  const handleAutomationChange = (
    setting: keyof Settings["automation"],
    value: boolean
  ) => {
    onSettingsChange({
      automation: {
      ...
      >
        <header className="flex items-center justify-between p-4 border-b border-slate-700 relative">
          {view !== "main" && (
            <button
              ...
              className="absolute left-4 p-2 rounded-full hover:bg-slate-700"
            >
              <ArrowLeftIcon className="w-6 h-6 text-slate-600" />
            </button>
          )}
          <h2 className="text-sm font-bold text-slate-400 w-full text-center">
            {titles[view]}
          </h2>
          <button
            onClick={handleClose}
            className="p-2 rounded-full hover:bg-slate-700 absolute right-4"
          >
        ...
        <div className="relative h-[480px] overflow-hidden">
          ...
          >
            <fieldset className="space-y-4">
              <legend className="sr-only">
                {t("settings.automation.legend")}
              </legend>
              <div className="flex items-center justify-between">
                <div>
                  <label
                    htmlFor="autoCheckShortPhrases"
                    className="text-sm text-slate-200"
                  >
                    {t("settings.automation.autoCheckShortPhrases.label")}
                  </label>
                  <p className="text-xs text-slate-400">
                    {t("settings.automation.autoCheckShortPhrases.description")}
                  </p>
                </div>
                <div>
                  <button
              ...
              <div className="flex items-center justify-between">
                <div>
                  <label
                    htmlFor="learnNextPhraseHabit"
                    className="text-sm text-slate-200"
                  >
                    {t("settings.automation.learnNextPhraseHabit.label")}
                  </label>
                  <p className="text-xs text-slate-400">
                    {t("settings.automation.learnNextPhraseHabit.description")}
                  </p>
                </div>
                <div>
                  <button
            ...
            <div className="bg-slate-900/50 p-4 rounded-lg space-y-4 h-[85%] flex flex-col justify-between overflow-y-auto">
              {categories.map((category) => (
                ...
                >
                  <label
                    htmlFor={category.id}
                    className="text-sm text-slate-300 flex items-center font-light"
                  >
                    <span
                      className={`min-w-3 min-h-3 rounded-full mr-2 ${category.color}`}
                    ></span>
                    {category.name}
                  </label>
                  <div>
            ...
            {practiceAnalyticsSummary.totals.totalCards > 0 ? (
                ...
                <div className="grid gap-3 grid-cols-2">
                  <AnalyticsCard
                    ...
                    primary={numberFormatter.format(
                      practiceAnalyticsSummary.totals.totalCards
                    )}
                    secondary={t("settings.analytics.srs.masteryProgress", {
                      percent:
                        practiceAnalyticsSummary.totals.masteryProgressPercent,
                    })}
                  />
                  <AnalyticsCard
                    label={t("settings.analytics.srs.mastered")}
                    primary={numberFormatter.format(
                      practiceAnalyticsSummary.totals.mastered
                    )}
                    secondary={t("settings.analytics.srs.learning", {
                      count: numberFormatter.format(
                        practiceAnalyticsSummary.totals.learning
                      ),
                    })}
                  />
                  <AnalyticsCard
                    ...
                    primary={numberFormatter.format(
                      practiceAnalyticsSummary.totals.dueToday
                    )}
                    secondary={t("settings.analytics.srs.overdue", {
                      count: numberFormatter.format(
                        practiceAnalyticsSummary.totals.overdue
                      ),
                    })}
                  />
                  <AnalyticsCard
                    label={t("settings.analytics.srs.accuracy")}
                    primary={formatPercentValue(
                      practiceAnalyticsSummary.accuracy.overall
                    )}
                    secondary={t("settings.analytics.srs.reviews", {
                      count: numberFormatter.format(
                        practiceAnalyticsSummary.accuracy.totalReviews
                      ),
                    })}
                  />
                </div>
                    ...
                    <p className="text-sm text-purple-200">
                      {t("settings.analytics.srs.streak", {
                        days: numberFormatter.format(
                          practiceAnalyticsSummary.accuracy.streakDays
                        ),
                      })}
                    </p>
                  </div>
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-slate-300">
                        {t("settings.analytics.srs.overall")}
                      </span>
                    ...
                    <div className="flex justify-between">
                      <span className="text-slate-300">
                        {t("settings.analytics.srs.last7Days")}
                      </span>
                      <span className="text-slate-100 font-semibold">
                        {formatPercentValue(
                          practiceAnalyticsSummary.accuracy.last7Days
                        )}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-300">
                        {t("settings.analytics.srs.last30Days")}
                      </span>
                      <span className="text-slate-100 font-semibold">
                        {formatPercentValue(
                          practiceAnalyticsSummary.accuracy.last30Days
                        )}
                      </span>
                    </div>
                  </div>
                ...
                <div className="bg-slate-900/40 border border-slate-700/60 rounded-xl p-4 shadow-inner">
                  ...
                  <div className="space-y-2">
                    {practiceAnalyticsSummary.levels.map((level) => {
                      const percent =
                        srsLevelsMaxCount > 0
                          ? (level.count / srsLevelsMaxCount) * 100
                          : 0;
                      return (
                        <div
                          key={level.level}
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <span className="text-xs text-slate-400">
                              {numberFormatter.format(cat.total)}{" "}
                              {t("settings.analytics.srs.cards")}
                            </span>
                          </div>
                          <div className="grid grid-cols-3 gap-2 text-xs">
                            <div>
                              <p className="text-slate-400">
                                {t("settings.analytics.srs.mastered")}
                              </p>
                              <p className="text-slate-100 font-semibold">
                                {numberFormatter.format(cat.mastered)}
                              </p>
                            </div>
                            <div>
                              <p className="text-slate-400">
                                {t("settings.analytics.srs.avgLevel")}
                              </p>
                              <p className="text-slate-100 font-semibold">
                            ...
                            <div>
                              <p className="text-slate-400">
                                {t("settings.analytics.srs.accuracy")}
                              </p>
                              <p className="text-slate-100 font-semibold">
                                {formatPercentValue(cat.accuracy)}
                              </p>
                            </div>
                          </div>
                        </div>
                ...
                <div className="bg-slate-900/40 border border-slate-700/60 rounded-xl p-4 shadow-inner">
                  ...
                  {practiceAnalyticsSummary.leeches.length > 0 ? (
                    <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                      {practiceAnalyticsSummary.leeches
                        .slice(0, 10)
                        .map((leech) => (
                          <div
                            key={leech.phraseId}
                          ...
                          >
                            ...
                            <span className="ml-2 px-2 py-1 bg-red-900/40 text-red-300 text-xs font-semibold rounded">
                              {leech.lapses}{" "}
                              {t("settings.analytics.srs.lapses")}
                            </span>
                          </div>
                        ))}
                    </div>
              ...
              <div className="h-full flex flex-col justify-center items-center text-center space-y-3 text-slate-400">
                <p className="text-base font-semibold">
                  {t("settings.analytics.srs.emptyState")}
                </p>
                <p className="text-sm text-slate-500 max-w-xs">
                  {t("settings.analytics.srs.emptyStateHint")}
                </p>
              </div>
            )}
          </div>
