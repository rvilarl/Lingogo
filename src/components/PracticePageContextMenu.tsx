...
const PracticePageContextMenu: React.FC<PracticePageContextMenuProps> = ({
  ...
  const renderMenuItems = () => {
      ...
      ? [
          {
            ...
            icon: <InfoIcon className="w-5 h-5 mr-3 text-slate-300" />,
            action: () => handleAction(() => onOpenWordAnalysis(phrase, word)),
            condition: !!analysis,
          },
          {
            label: t('assistant.contextMenu.createWordCard'),
            icon: <PlusIcon className="w-5 h-5 mr-3 text-slate-300" />,
            action: () => handleAction(handleCreateCard),
            condition: !!analysis,
            loading: isCreatingCard,
          },
          {
            label: t('modals.wordAnalysis.actions.openVerb'),
            icon: <TableIcon className="w-5 h-5 mr-3 text-slate-300" />,
            action: () => handleAction(() => onOpenVerbConjugation(analysis!.verbDetails!.infinitive)),
            condition: !!analysis?.verbDetails,
          },
          {
            label: t('modals.wordAnalysis.actions.openNoun'),
            icon: <TableIcon className="w-5 h-5 mr-3 text-slate-300" />,
            action: () => handleAction(() => onOpenNounDeclension(analysis!.word, analysis!.nounDetails!.article)),
            condition: !!analysis?.nounDetails,
          },
          {
            label: t('modals.wordAnalysis.actions.openAdjective'),
            icon: <TableIcon className="w-5 h-5 mr-3 text-slate-300" />,
            action: () => handleAction(() => onOpenAdjectiveDeclension(analysis!.baseForm || analysis!.word)),
    ...
    const phraseSpecificItems = [
      {
        label: t('assistant.contextMenu.goToList'),
        icon: <ListIcon className="w-5 h-5 mr-3 text-slate-300" />,
        action: () => handleAction(() => onGoToList(phrase)),
        condition: true,
      },
      {
        label: t('assistant.contextMenu.discussTranslation'),
        icon: <MessageQuestionIcon className="w-5 h-5 mr-3 text-slate-300" />,
        action: () => handleAction(() => onDiscuss(phrase)),
        condition: true,
      },
      {
        label: t('common.actions.delete'),
        icon: <TrashIcon className="w-5 h-5 mr-3" />,
        action: () => handleAction(() => onDelete(phrase.id)),
        condition: true,
        isDestructive: true,
      },
    ];
    ...
    return (
      <>
        ...
        <div className="p-1">
            ...
            wordSpecificItems
              ...
              .map((item, index) => (
                <button
                  ...
                  className="w-full flex items-center px-3 py-2 text-left text-sm hover:bg-slate-600/70 transition-colors rounded-md disabled:opacity-50"
                >
                  {item.loading ? (
                    <div className="flex space-x-1 items-center justify-center text-slate-300 mr-3">
                      <div className="w-2 h-2 bg-current rounded-full animate-pulse"></div>
          ...
          {word && !isAnalysisLoading && wordSpecificItems.filter((item) => item.condition).length > 0 && (
            <hr className="border-slate-600 my-1 mx-2" />
          )}
          ...
          {phraseSpecificItems.map((item, index) => (
            <button
              key={index}
              onClick={item.action}
              className={`w-full flex items-center px-3 py-2 text-left text-sm hover:bg-slate-600/70 transition-colors rounded-md ${item.isDestructive ? 'text-red-400' : ''}`}
            >
              {item.icon}
              <span>{item.label}</span>
            </button>
  ...
  return (
    <>
      ...
      <div className="fixed z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-slate-700/90 backdrop-blur-sm border border-slate-600 rounded-lg shadow-2xl animate-fade-in-center text-white w-64 overflow-hidden">
        {renderMenuItems()}
      </div>
    </>
  );
