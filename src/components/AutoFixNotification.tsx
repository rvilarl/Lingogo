...
import { IoCheckmarkCircleOutline, IoClose, IoCloseCircleOutline } from 'react-icons/io5';
    ...
    <div className="fixed bottom-4 right-4 z-50 w-96 animate-slide-up">
      <div className="bg-slate-800 rounded-lg shadow-2xl border border-slate-700 overflow-hidden">
        ...
        <div className="p-4">
          ...
          {status === 'fixing' && progress && (
            <div className="space-y-3">
              ...
              {progress.currentPhrase && (
                <div className="text-xs text-slate-400 truncate">Processing: {progress.currentPhrase}</div>
              )}
            </div>
          )}
          ...
          {status === 'error' && error && (
            <div className="space-y-2">
              <p className="text-sm text-red-300">Failed to auto-fix phrases:</p>
              <p className="text-xs text-red-200 bg-red-900/30 p-2 rounded">{error}</p>
            </div>
          )}
        </div>
