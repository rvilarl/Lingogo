...
const ChatMessageContent: React.FC<{
  ...
  const handleWordClick = (contextText: string, word: string) => {
    const proxyPhrase = {
      ...basePhrase,
      id: `proxy_discuss_${contextText.slice(0, 5)}`,
      text: { ...basePhrase.text, learning: contextText },
    };
        ...
        <div className="p-1 border-t border-slate-700 flex-shrink-0 bg-slate-800/80 backdrop-blur-sm">
          {latestSuggestion && (
            <div className="bg-slate-700/50 p-3 rounded-lg mb-3">
              ...
              <p className="font-semibold text-slate-200">
                {latestSuggestion.native} → {latestSuggestion.learning}
              </p>
              <button
                onClick={handleAccept}
                className="w-full mt-2 text-sm flex items-center justify-center py-2 bg-green-600 hover:bg-green-700 rounded-md font-bold text-white"
              >
          ...
          >
            <input
              ...
              placeholder={
                isListening ? t('modals.chat.placeholders.listening') : t('modals.discussTranslation.placeholder')
              }
            ...
            <button
              ...
              className={`p-3 rounded-lg flex-shrink-0 ${isListening ? 'bg-red-600 animate-pulse' : 'bg-slate-600'} hover:bg-slate-500`}
              disabled={isLoading}
            >
