...
import { getLanguageName, LANGUAGE_OPTIONS } from '../i18n/languageMeta.ts';
...
const DevLanguageSelector: React.FC<DevLanguageSelectorProps> = ({
  visible,
  selectedLanguage,
  onSelect,
  onUseSystem,
    ...
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/90 backdrop-blur-sm px-4">
      <div className="w-full max-w-3xl rounded-2xl border border-slate-800 bg-slate-900/95 shadow-2xl flex flex-col max-h-[min(92vh,720px)]">
        <div className="border-b border-slate-800 px-6 py-5 space-y-2">
          <p className="text-xs uppercase tracking-[0.35em] text-slate-500">{t('devLanguageSelector.heading')}</p>
          <h1 className="text-2xl font-semibold text-slate-100">{t('devLanguageSelector.title')}</h1>
          <p className="text-sm text-slate-400">{t('devLanguageSelector.subtitle')}</p>
        </div>
        ...
        <div className="border-t border-slate-800 px-6 py-4 flex flex-wrap items-center justify-between gap-3 bg-slate-900/80">
          <p className="text-xs text-slate-500">{t('devLanguageSelector.note')}</p>
          <button
            type="button"
            onClick={onUseSystem}
