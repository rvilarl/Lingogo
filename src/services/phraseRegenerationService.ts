...
import { extractEmbeddedRomanization, quickFixPhrase } from './phraseValidationService';
...
Return ONLY the romanization text, nothing else.`;
    ...
    const romanization = typeof response === 'string' ? response.trim() : response.text?.trim() || '';
